    ...
    const getSaasChartData = (saas: SaaSBuyersData) => {
        ...
        return Object.entries(buyersByDay)
            .map(([date, buyers]) => ({
                date,
                count: buyers.length,
                revenue: buyers.reduce((sum, buyer) => sum + (buyer.price || 0), 0)
            }))
    ...
    return (
        <div className="bg-white min-h-screen">
            <div className="max-w-5xl mx-auto py-6 px-4">
                <h2 className="text-xl font-bold text-black mb-6">SaaS Buyers Dashboard</h2>
                ...
                {loading && !buyersData ? (
                    ...
                    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mb-6">
                        <p className="text-center py-4 text-gray-500">{error}</p>
                    </div>
                ) : !buyersData || buyersData.length === 0 ? (
                    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mb-6">
                        <p className="text-center py-4 text-gray-500">No SaaS data available</p>
                    </div>
                ) : (
                    buyersData.map((saas) => {
                        ...
                        const totalBuyers = chartData.reduce((sum, day) => sum + day.count, 0);
                        const totalRevenue = chartData.reduce((sum, day) => sum + day.revenue, 0);

                        return (
                            <div key={saas.saasName} className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm mb-6">
                                <div
                                    className="flex justify-between items-center cursor-pointer"
                                    onClick={() => toggleSaasExpand(saas.saasName)}
                                >
                                    <div>
                                        <h3 className="text-lg font-medium text-black">{saas.saasName}</h3>
                                        <div className="h-1 w-16 bg-gray-500 mt-2 mb-2"></div>
                                        <p className="text-sm text-gray-500">Buyer activity</p>
                                    </div>
                                ...
                                {isExpanded && (
                                    <div className="mt-6 animate-fadeIn">
                                        {chartData.length === 0 ? (
                                            <div className="text-center py-8 text-gray-500">No buyer data available for the last 30 days</div>
                                        ) : (
                                            <>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                                    <div className="bg-gray-50 p-4 rounded-md">
                                                        <h4 className="text-sm font-medium text-gray-500 mb-1">Total Buyers (30 days)</h4>
                                                        <p className="text-2xl font-semibold text-black">{totalBuyers}</p>
                                                    </div>
                                                    <div className="bg-gray-50 p-4 rounded-md">
                                                        <h4 className="text-sm font-medium text-gray-500 mb-1">Revenue (30 days)</h4>
                                                        <p className="text-2xl font-semibold text-black">${totalRevenue.toFixed(2)}</p>
                                                    </div>
                                                </div>

                                                <h4 className="text-sm font-medium text-gray-700 mb-3">Buyers Last 30 Days</h4>
                                                        ...
                                                        {chartData.map((day, index) => (
                                                            <div key={index} className="group relative flex flex-col items-center flex-1">
                                                                ...
                                                                <div className="opacity-0 group-hover:opacity-100 absolute bottom-full mb-2 -translate-x-1/2 bg-neutral-800 text-white text-xs py-1 px-2 rounded pointer-events-none whitespace-nowrap">
                                                                    {day.count} buyers<br />${day.revenue.toFixed(2)} revenue<br />
                                                ...
                                                <div className="mt-8">
                                                    <h4 className="text-sm font-medium text-gray-700 mb-3">Latest Buyers</h4>
                                                        ...
                                                        <table className="min-w-full divide-y divide-gray-200">
                                                            ...
                                                            <tbody className="bg-white divide-y divide-gray-200">
                                                                {saas.buyers
                                                                    ...
                                                                    .slice(0, 5)
                                                                    .map((buyer, buyerIndex) => (
                                                                        <tr key={buyerIndex}>
                                                                            <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{buyer.email}</td>
                                                                            <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">{buyer.plan}</td>
                                                                            <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900">
                                                                                {buyer.price ? `$${buyer.price.toFixed(2)}` : '-'}
                                                                            </td>
                                                                            <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500">
                                                                                {new Date(buyer.time).toLocaleDateString()}
                                                                            </td>
                                                                        </tr>
                                                                    ))
                                                                }
