...
export const HomePage = () => {
    ...
    const [newTier, setNewTier] = useState<Partial<TierType>>({
        tier: '',
        price: 0,
    });
    const [editingTierIndex, setEditingTierIndex] = useState<number | null>(
        null
    );
    ...
    const { totalUsers, totalTiers, averageTierPrice } = useMemo(() => {
        if (!userData || userData.length === 0)
            return { totalUsers: 0, totalTiers: 0, averageTierPrice: '0.00' };

        const totalUsers = userData.length;
        const totalTiers = userData.reduce(
            (acc, user) => acc + user.tiers.length,
            0
        );
        ...
        const averageTierPrice = totalTiers
            ? (
                  userData
                      .flatMap((user) => user.tiers)
                      .reduce((acc, tier) => acc + tier.price, 0) / totalTiers
              ).toFixed(2)
            : '0.00';
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {userData?.length ? (
                                        userData.map((user) => {
                                            const minPrice = user.tiers.length
                                                ? Math.min(
                                                      ...user.tiers.map(
                                                          (tier) => tier.price
                                                      )
                                                ...
                                                : 0;
                                            const maxPrice = user.tiers.length
                                                ? Math.max(
                                                      ...user.tiers.map(
                                                          (tier) => tier.price
                                                      )
                                                        ...
                                                        {user.tiers.map(
                                                            (tier) => (
                                                                <span
                                                                    ...
                                                                    className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-700 mr-1"
                                                                >
                                                                    {tier.tier}{' '}
                                                                    ($
                                                                    {tier.price}
                        ...
                        <div className="border-b border-gray-200 pb-4 mb-4">
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex">
                                    ...
                                    <button
                                        onClick={() =>
                                            copyToClipboard(editingUser._id)
                                        }
