...
export async function POST(req: NextRequest) {
	await connectDB();
	try {
		const { publicKey, saasName, logoUrl, address, tiers } = await req.json();
		...
		// Create the User
		const newUser = await User.create({ publicKey, saasName, logoUrl, address });

		// Add User-specific Tiers
